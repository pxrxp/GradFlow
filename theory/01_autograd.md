# Theory Chapter 1: The Autograd Engine
...
### Base Case: `dL/dL = 1`
When `backward()` is called, we set `loss.grad = 1`. This "seed" allows the chain rule to propagate backwards.

## 3. Operation Mapping

| Op | Derivative ($\frac{\partial z}{\partial x}$) | `_backward` implementation |
| :--- | :--- | :--- |
| $z = x + y$ | $1$ | `x.grad += 1.0 * grad` |
| $z = x \cdot y$ | $y$ | `x.grad += y * grad` |
| $z = x^n$ | $n \cdot x^{n-1}$ | `x.grad += (n * x**(n-1)) * grad` |
| $z = \text{ReLU}(x) = \max(0,x)$ | $1$ if $x > 0$ else $0$ | `x.grad += (self.data > 0) * grad` |
